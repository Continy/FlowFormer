...
class CrossAttentionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.q, self.k, self.v = nn.Linear(
            query_token_dim, qk_dim,
            bias=True), nn.Linear(tgt_token_dim, qk_dim,
                                  bias=True), nn.Linear(tgt_token_dim,
                                                        v_dim,
                                                        bias=True)
        ...
        self.ffn = nn.Sequential(nn.Linear(query_token_dim, query_token_dim),
    ...
    def forward(self, query, key, value, memory, query_coord, patch_size,
                size_h3w3):
        """
        ...
        query_coord = query_coord.view(B, 2, -1).permute(
            0, 2, 1)[:, :, None, :].contiguous().view(B * H1 * W1, 1, 2)
        if self.pe == 'linear':
            query_coord_enc = LinearPositionEmbeddingSine(query_coord,
                                                          dim=self.dim)
        elif self.pe == 'exp':
            query_coord_enc = ExpPositionEmbeddingSine(query_coord,
                                                       dim=self.dim)
...
class MemoryDecoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.flow_token_encoder = nn.Sequential(
            nn.Conv2d(81 * cfg.cost_heads_num, dim, 1, 1), nn.GELU(),

...
@torch.no_grad()
def validate_tartanair(model):
    ''' Peform validation using the TartanAir P000 split '''
    model.eval()
    val_dataset = datasets.TartanAir(folderlength=1)
    out_list, epe_list = [], []
    for val_id in range(len(val_dataset)):
        image1, image2, flow_gt, valid_gt = val_dataset[val_id]
        image1 = image1[None].cuda()
    ...
    print("Validation TartanAir: %f, %f" % (epe, f1))
    return {'tartanair-epe': epe, 'tartanair-f1': f1}
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        if args.dataset == 'chairs':
            ...
            validate_tartanair(model.module)
